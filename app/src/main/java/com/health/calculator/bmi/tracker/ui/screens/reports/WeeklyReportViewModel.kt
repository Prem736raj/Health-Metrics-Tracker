...
class WeeklyReportViewModel @Inject constructor(
    ...
    fun shareReport(@ApplicationContext context: Context) {
        ...
        if (!hasShareSelection(state)) return
    ...
    private fun hasShareSelection(state: WeeklyReportUiState): Boolean =
        state.shareIncludeScore ||
            state.shareIncludeWeight ||
            state.shareIncludeBmi ||
            state.shareIncludeBp ||
            state.shareIncludeWater ||
            state.shareIncludeCalories ||
            state.shareIncludeExercise
}
